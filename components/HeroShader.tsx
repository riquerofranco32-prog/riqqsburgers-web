...
export default function HeroShader() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // ── Aurora layers ──────────────────────────────────────────────────────
      const SLICES = Math.ceil(W / 6); // coarser slices = ~2x fewer gradient calls
      ...
      for (const layer of layers) {
        ...
        for (let i = 0; i <= SLICES; i++) {
          ...
          const pull =
            Math.exp(-distX * distX * 8) * (my - layer.yOffset) * 0.18;

          const noise = smoothNoise(
            xNorm * Math.PI * layer.wavelength,
            layerT,
            layer.phase,
          );
        ...
        for (let i = 0; i < points.length - 1; i++) {
          ...
          grad.addColorStop(
            0.3,
            `rgba(${col.r},${col.g},${col.b},${(layer.opacity * 0.8).toFixed(3)})`,
          );
          grad.addColorStop(
            0.5,
            `rgba(${col.r},${col.g},${col.b},${layer.opacity.toFixed(3)})`,
          );
          grad.addColorStop(
            0.7,
            `rgba(${col.r},${col.g},${col.b},${(layer.opacity * 0.8).toFixed(3)})`,
