...
export default function CheckoutModal({ isOpen, onClose, cart, onClearCart, tenant }: CheckoutModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setLoading(false)
      setTouched({})
      setForm({ name: '', lastname: '', phone: '', address: '', notes: '', delivery: 'pickup', payment: 'cash' })
    }
  }, [isOpen])
  ...
  const addressError = touched.address && form.delivery === 'delivery' && !form.address ? 'Ingresá la dirección' : ''
      ...
      >
        ...
        {done ? (
          ...
          <div style={{ textAlign: 'center', padding: '32px 24px 40px' }}>
            ...
            <h2 style={{ color: 'var(--text-primary)', fontSize: 22, fontWeight: 800, marginBottom: 8 }}>
              ¡Pedido enviado!
            </h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 24, lineHeight: 1.5 }}>
              Se abrió WhatsApp con tu pedido. Guardá el código para hacer el seguimiento.
            </p>
            <div style={{
              background: 'var(--surface-2)',
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', marginBottom: 8, textTransform: 'uppercase' }}>
                Número de pedido
              </p>
              <span style={{ color: accent, fontSize: 32, fontWeight: 800, letterSpacing: '0.2em', fontFamily: 'monospace' }}>
                {orderRef}
              </span>
            </div>
          ...
          <div style={{ padding: '20px 20px 32px' }}>
            <h2 style={{ color: 'var(--text-primary)', fontSize: 20, fontWeight: 800, marginBottom: 6 }}>
              Completá tu pedido
            </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input
                    style={{ ...inputBase, borderColor: nameError ? '#ef4444' : 'var(--border)' }}
                    placeholder="Juan"
                    value={form.name}
                ...
                <div>
                  ...
                  <input
                    style={{ ...inputBase, borderColor: lastnameError ? '#ef4444' : 'var(--border)' }}
                    placeholder="García"
                    value={form.lastname}
                    ...
                    onFocus={e => (e.target.style.borderColor = accent)}
                  />
                  {lastnameError && <span style={errorStyle}>{lastnameError}</span>}
                </div>
              </div>
                  ...
                  {([
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: form.delivery === opt.value ? `${accent}18` : 'var(--surface-2)',
                        color: form.delivery === opt.value ? accent : 'var(--text-secondary)',
                        cursor: 'pointer',
                        textAlign: 'left',
              ...
              {form.delivery === 'delivery' && (
                <div>
                  ...
                  <input
                    style={{ ...inputBase, borderColor: addressError ? '#ef4444' : 'var(--border)' }}
                    ...
                    onFocus={e => (e.target.style.borderColor = accent)}
                  />
                  {addressError && <span style={errorStyle}>{addressError}</span>}
                </div>
              )}
                  ...
                  {([
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: form.payment === opt.value ? `${accent}18` : 'var(--surface-2)',
                        color: form.payment === opt.value ? accent : 'var(--text-secondary)',
                        cursor: 'pointer',
                        fontSize: 13,
                        fontWeight: 600,
                        transition: 'all 0.15s',
              ...
              <div style={{
                ...
                {cart.map(i => (
                  <div key={i.id} style={{
                    ...
                    color: 'var(--text-secondary)', fontSize: 13, marginBottom: 6,
                  }}>
                ...
                {deliveryCost > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', fontSize: 13, marginBottom: 6 }}>
                    <span>🛵 Envío</span>
                ...
                <div style={{
                  display: 'flex', justifyContent: 'space-between',
                  color: 'var(--text-primary)', fontWeight: 800, fontSize: 17,
