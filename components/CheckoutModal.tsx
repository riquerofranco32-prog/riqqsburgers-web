...
function buildMessage(form: FormState, cart: CartItem[]): string {
  const now = new Date()
  ...
  const envio = form.tipoServicio === 'domicilio' ? COSTO_ENVIO : 0
  ...
  const mpLine =
    form.metodoPago === 'mercadopago' ? `Mercado Pago ${total}` : ''

  let msg = `Vengo de ${RESTAURANT_NAME}
${orderId}
🗓️ ${fecha} ⏰ ${hora}
...
function Input({
  ...
  return (
    <div>
      <div className={`flex items-center bg-[#111] border rounded-xl overflow-hidden transition-colors ${error ? 'border-red-500' : 'border-[#2a2a2a] focus-within:border-[#f5c518]'}`}>
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-semibold border transition-all min-h-[48px] ${
            value === o.value
              ...
              : 'bg-transparent text-[#888] border-[#2a2a2a] hover:border-[#f5c518]/40 hover:text-white'
          }`}
        >
...
function SuccessView({ total, metodoPago, onClose }: {
  total: number; metodoPago: string; onClose: () => void
}) {
  return (
    <div className="flex flex-col items-center gap-5 py-6 text-center">
      ...
      {metodoPago === 'mercadopago' && MP_LINK && (
        <div className="w-full bg-[#009ee3]/10 border border-[#009ee3]/30 rounded-2xl p-4 flex flex-col gap-3">
          ...
          >
            <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M12.003 0C5.374 0 0 5.374 0 12.003 0 18.629 5.374 24 12.003 24 18.629 24 24 18.629 24 12.003 24 5.374 18.629 0 12.003 0zm5.99 13.674c-.346.92-1.168 1.527-2.14 1.527H9.147c-.972 0-1.794-.607-2.14-1.527l-.346-.92h10.678l-.346.92z"/></svg>
...
export function CheckoutModal({ cart, onClose }: {
  cart: CartItem[]
  onClose: () => void
}) {
  ...
  function handleSubmit() {
    if (!validate()) return
    const msg = buildMessage(form, cart)
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative bg-[#1a1714] w-full max-w-lg rounded-t-3xl flex flex-col max-h-[92dvh] shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-[#2a2a2a] flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-[#111] flex items-center justify-center text-[#888] hover:text-white transition-colors"
          >
            ×
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {submitted ? (
            <SuccessView total={total} metodoPago={form.metodoPago} onClose={onClose} />
          ) : (
              ...
              <div className="bg-[#111] rounded-2xl p-4 flex flex-col gap-2 border border-[#2a2a2a]">
                {cart.map(c => (
                  <div key={c.item.id} className="flex justify-between text-sm">
                    <span className="text-[#aaa]">{c.cantidad}× {c.item.nombre}</span>
                    <span className="text-white font-semibold">{fmtARS(c.item.precio * c.cantidad)}</span>
                  </div>
                ))}
                <div className="border-t border-[#2a2a2a] mt-1 pt-2 flex flex-col gap-1">
                  ...
                  {form.tipoServicio === 'domicilio' && (
                    <div className="flex justify-between text-sm text-[#888]">
                      <span>Envío</span><span>{fmtARS(COSTO_ENVIO)}</span>
                    </div>
                  )}
                  <div className="flex justify-between font-bold text-base mt-1">
                    <span>Total</span>
                    <span className="text-[#f5c518]">{fmtARS(total)}</span>
                  </div>
                </div>
              </div>
        ...
        {!submitted && (
          <div className="px-5 py-4 border-t border-[#2a2a2a] flex-shrink-0">
            <button
              ...
              className="w-full bg-[#f5c518] text-black font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-amber-400 active:scale-[0.98] transition-all text-base min-h-[56px]"
            >
