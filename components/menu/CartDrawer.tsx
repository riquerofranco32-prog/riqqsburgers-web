...
export function CartDrawer({ tenantId, whatsappNumber, onClose, onCheckout }: CartDrawerProps) {
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-zinc-800 flex flex-col gap-3 flex-shrink-0">
            ...
            <button
              ...
              disabled={sending}
              className="w-full bg-green-500 hover:bg-green-400 active:scale-[0.98] text-white font-bold py-3.5 rounded-2xl flex items-center justify-center gap-2.5 transition-all disabled:opacity-60"
            >
              <WAIcon />
              {sending ? 'Abriendo WhatsApp...' : 'Pedir por WhatsApp'}
            </button>
            <p className="text-center text-xs text-zinc-500">
              Te redirigimos a WhatsApp para confirmar y pagar
            </p>
          </div>
        )}
      </div>
