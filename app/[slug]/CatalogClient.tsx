...
export default function CatalogClient({ restaurant }: { restaurant: Restaurant }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ;['--bg','--surface','--surface-2','--accent','--text-primary','--text-secondary','--border']
  ...
  function scrollToCategory(catId: string) {
    ...
    catBtnRefs.current[catId]?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' })

    // Release lock after scroll settles
    setTimeout(() => { isScrollingToCat.current = false }, 800)
  }
  ...
  function onDrawerTouchEnd(e: React.TouchEvent) {
    ...
    if (delta > 80) { setCartOpen(false); vibrate(30) }
  }
  ...
  const searchResults = restaurant.menu.categories
    .flatMap(c => c.items.map(item => ({ ...item, _catEmoji: c.emoji, _catName: c.name })))
    .filter(i =>
    ...
    return (
      <div
        ...
        style={{
          ...
          background: SURFACE, borderRadius: 16, padding: 14, marginBottom: 8,
          opacity: soldOut ? 0.55 : 1,
          boxShadow: qty > 0 ? `0 3px 16px ${accent}22` : '0 1px 4px rgba(0,0,0,0.06)',
        ...
        onTouchStart={e => { if (!soldOut) e.currentTarget.style.transform = 'scale(0.985)' }}
        onTouchEnd={e => { e.currentTarget.style.transform = '' }}
        onTouchCancel={e => { e.currentTarget.style.transform = '' }}
      >
        {/* Left: text */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <span style={{ fontWeight: 700, fontSize: 15, color: TEXT1, lineHeight: 1.3, display: 'block', marginBottom: 3 }}>
            {item.name}
          </span>

          {item.badge && item.badge !== '' && (
            <div style={{ marginBottom: 5 }}><Badge badge={item.badge} /></div>
          )}

          {item.description && (
            <p style={{
              fontSize: 12, color: TEXT2, margin: '0 0 8px', lineHeight: 1.45,
              display: '-webkit-box',
              WebkitLineClamp: 2,
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {!item.image && !soldOut && qty > 0 && (
              ...
              >
                <button onClick={() => removeItem(item)} style={{ width: 28, height: 28, borderRadius: '50%', background: BORDER, border: 'none', color: TEXT1, fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>−</button>
                <span style={{ fontWeight: 700, fontSize: 14, minWidth: 18, textAlign: 'center', color: TEXT1 }}>{qty}</span>
                <button onClick={() => addItem(item)} style={{ width: 28, height: 28, borderRadius: '50%', background: accent, border: 'none', color: onAccent, fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>+</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          {item.image ? (
            <img src={item.image} alt={item.name} style={{ width: 90, height: 90, borderRadius: 12, objectFit: 'cover', display: 'block' }} />
          ) : (
            <div style={{
            ...
            qty === 0 ? (
              ...
              <div
                ...
                style={{
                  ...
                  background: SURFACE, borderRadius: 20, padding: '3px 6px',
                  boxShadow: `0 2px 10px ${accent}40`,
                  border: `1.5px solid ${accent}30`,
                }}
              >
                <button onClick={() => removeItem(item)} style={{ width: 24, height: 24, borderRadius: '50%', background: accent + '18', border: 'none', color: accent, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>−</button>
                <span style={{ fontWeight: 800, fontSize: 13, minWidth: 16, textAlign: 'center', color: TEXT1 }}>{qty}</span>
                <button onClick={() => addItem(item)} style={{ width: 24, height: 24, borderRadius: '50%', background: accent, border: 'none', color: onAccent, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>+</button>
              </div>
            )
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <header style={{ position: 'relative', height: 220, overflow: 'hidden', flexShrink: 0 }}>
        ...
        <div style={{ position: 'absolute', top: 14, right: 16, zIndex: 2 }}>
          <span style={{
            ...
            color: '#fff', backdropFilter: 'blur(8px)', letterSpacing: '0.02em',
          }}>
            <span style={{
              width: 6, height: 6, borderRadius: '50%', background: '#fff',
              display: 'inline-block',
              animation: restaurant.is_open ? 'blink 2s ease-in-out infinite' : undefined,
            }} />
        ...
        <div style={{
          ...
          {restaurant.tagline && (
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', fontStyle: 'italic', marginBottom: 10, textAlign: 'center' }}>
              {restaurant.tagline}
            </p>
          )}
          {infoItems.length > 0 && <InfoRotator items={infoItems} accent={accent} />}
        </div>
      </header>
      ...
      {!restaurant.is_open && (
        <div style={{
          background: 'rgba(220,38,38,0.07)', borderBottom: '1px solid rgba(220,38,38,0.12)',
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 40, background: BG, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
        {/* Search */}
        <div style={{ padding: '10px 16px 6px' }}>
          <div style={{ maxWidth: 640, margin: '0 auto', position: 'relative' }}>
            <Search size={15} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: TEXTM, pointerEvents: 'none' }} />
            <input
              type="text"
              value={searchQuery}
            ...
            {searchQuery && (
              <button
                ...
                style={{
                  ...
                  background: SURFACE2, border: 'none', width: 22, height: 22, borderRadius: '50%',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: isActive ? `0 4px 16px ${accent}45` : '0 1px 3px rgba(0,0,0,0.05)',
                ...
                >
                  ...
                  {cat.items.length > 0 && (
                    <span style={{
                      ...
                      background: isActive ? 'rgba(255,255,255,0.28)' : SURFACE2,
      ...
      {/* ── Products ─────────────────────────────────────────────────────────── */}
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '8px 12px 120px' }}>

        {/* ── Search results ── */}
        {searchQuery ? (
          <>
            <p style={{ fontSize: 12, color: TEXTM, marginBottom: 10, paddingLeft: 4 }}>
              {searchResults.length} resultado{searchResults.length !== 1 ? 's' : ''} para &ldquo;{searchQuery}&rdquo;
            </p>
            {searchResults.length > 0 ? searchResults.map(item => (
              <ProductCard
                key={item.id} item={item}
                catEmoji={(item as typeof item & { _catEmoji: string })._catEmoji ?? '🍽️'}
                accent={accent} onAccent={onAccent}
                SURFACE={SURFACE} SURFACE2={SURFACE2} BORDER={BORDER} TEXT1={TEXT1} TEXT2={TEXT2}
              />
            )) : (
              <div style={{ textAlign: 'center', padding: '64px 0', color: TEXTM }}>
                <div style={{ width: 72, height: 72, borderRadius: '50%', background: SURFACE2, margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>🔍</div>
                <p style={{ fontSize: 15, fontWeight: 700, color: TEXT2, marginBottom: 6 }}>Sin resultados</p>
                <p style={{ fontSize: 13 }}>No encontramos &ldquo;{searchQuery}&rdquo; en el menú</p>
              </div>
            )}
          </>
        ) : (
          ...
          restaurant.menu.categories.map(cat => (
            ...
            >
              ...
              <div style={{
                ...
                <span style={{ fontSize: 17, fontWeight: 800, color: TEXT1 }}>{cat.name}</span>
                <div style={{ flex: 1, height: 1, background: BORDER, marginLeft: 4 }} />
                <span style={{ fontSize: 11, color: TEXTM }}>{cat.items.length}</span>
              </div>
              ...
              {cat.items.length > 0 ? cat.items.map(item => (
                ...
                <div style={{ textAlign: 'center', padding: '32px 0', color: TEXTM }}>
      ...
      {totalItems > 0 && !selectedItem && !cartOpen && (
        <button
          onClick={() => { vibrate(50); setCartOpen(true) }}
          style={{
            position: 'fixed',
            ...
            WebkitTapHighlightColor: 'transparent',
            maxWidth: 608, marginLeft: 'auto', marginRight: 'auto',
            animation: cartBounce ? 'cartPop 0.35s cubic-bezier(0.36,0.07,0.19,0.97)' : 'cartEnter 0.3s cubic-bezier(0.22,1,0.36,1)',
          } as React.CSSProperties}
        >
          <span style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
            ...
            {hasDelivery && (
              <span style={{ fontSize: 10, opacity: 0.75 }}>+ envío {fmt(restaurant.delivery_cost)}</span>
            )}
          </div>
        </button>
      )}
        ...
        <>
          ...
          >
            ...
            <div style={{
              ...
              padding: '12px 20px 14px', borderBottom: `1px solid ${BORDER}`, flexShrink: 0,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <ShoppingBag size={18} style={{ color: accent }} />
                <span style={{ fontWeight: 800, fontSize: 17, color: TEXT1 }}>Tu pedido</span>
                <span style={{
                  background: accent, color: onAccent,
                  borderRadius: 20, padding: '1px 8px', fontSize: 12, fontWeight: 800,
                }}>
                  {totalItems}
                </span>
              </div>
              <button
                onClick={() => setCartOpen(false)}
                style={{
                  width: 32, height: 32, borderRadius: '50%', background: SURFACE2,
                  border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: TEXT2,
                }}
              >
                <X size={16} />
              </button>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10, WebkitOverflowScrolling: 'touch' }}>
              {cart.map(item => {
                const catEmoji = restaurant.menu.categories.find(c =>
                ...
                return (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: TEXT1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.name}
                      </p>
                      <p style={{ fontSize: 11, color: TEXTM }}>{fmt(item.price)} c/u</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 800, minWidth: 20, textAlign: 'center', color: TEXT1 }}>
                        {item.quantity}
                      </span>
                      <button
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, minWidth: 60, textAlign: 'right', color: TEXT1 }}>
            ...
            <div style={{ borderTop: `1px solid ${BORDER}`, flexShrink: 0, padding: '12px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 13, color: TEXT2, fontWeight: 600 }}>{fmt(subtotal)}</span>
              </div>
              {hasDelivery && (
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 13, color: TEXTM }}>Envío</span>
                  <span style={{ fontSize: 13, color: TEXT2 }}>{fmt(restaurant.delivery_cost)}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 8, borderTop: `1px solid ${BORDER}`, marginTop: 4 }}>
                <span style={{ fontWeight: 800, fontSize: 18, color: TEXT1 }}>Total</span>
        ...
        return (
          <>
            ...
            >
              ...
              {selectedItem.image ? (
                <img src={selectedItem.image} alt={selectedItem.name} style={{ width: '100%', height: 220, objectFit: 'cover', marginTop: 10 }} />
              ) : (
                <div style={{
                  width: '100%', height: 140, marginTop: 10,
                  background: `linear-gradient(135deg, ${accent}18, ${accent}06)`,
              ...
              <div style={{ padding: '20px 20px', paddingBottom: `max(28px, env(safe-area-inset-bottom, 28px))` }}>
                {selectedItem.badge && <div style={{ marginBottom: 10 }}><Badge badge={selectedItem.badge} /></div>}

                <h2 style={{ fontWeight: 800, fontSize: 22, color: TEXT1, marginBottom: 8, lineHeight: 1.2 }}>
                  {selectedItem.name}
                </h2>

                {selectedItem.description && (
                  <p style={{ fontSize: 14, color: TEXT2, lineHeight: 1.65, marginBottom: 16 }}>
                    {selectedItem.description}
                  </p>
                )}

                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                  <span style={{ fontWeight: 900, fontSize: 28, color: accent }}>{fmt(selectedItem.price)}</span>
                  {qty > 0 && (
                    <span style={{ fontSize: 12, color: TEXTM, background: SURFACE2, borderRadius: 8, padding: '4px 10px' }}>
                      En carrito: {qty}
                    </span>
                  )}
                </div>
                ...
                {qty === 0 ? (
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    <div style={{
                      ...
                      flex: 1, justifyContent: 'space-between', border: `1px solid ${BORDER}`,
                    }}>
                      <button onClick={() => removeItem(selectedItem)} style={{ width: 38, height: 38, borderRadius: '50%', background: BORDER, border: 'none', color: TEXT1, fontSize: 22, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, WebkitTapHighlightColor: 'transparent' }}>−</button>
                      <span style={{ fontWeight: 800, fontSize: 18, minWidth: 28, textAlign: 'center', color: TEXT1 }}>{qty}</span>
                      <button onClick={() => addItem(selectedItem)} style={{ width: 38, height: 38, borderRadius: '50%', background: accent, border: 'none', color: onAccent, fontSize: 22, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, WebkitTapHighlightColor: 'transparent' }}>+</button>
                    </div>
                    <button
                      onClick={() => setSelectedItem(null)}
                      style={{
                        ...
                        color: TEXT2, cursor: 'pointer', whiteSpace: 'nowrap',
                        WebkitTapHighlightColor: 'transparent',
                      }}
                    >
                      Listo ✓
